...
function renderTasks(taskArray, tableBody) {
  ...
  taskArray.forEach((task, idx) => {
    ...
    countInput.min = 1;
    countInput.onchange = () => {
      task.count = parseInt(countInput.value) || 1;
      saveData();
    };
    ...
    btn.onclick = () => {
      ...
      renderTasks(taskArray, tableBody);
      saveData();
    };
...
addWorkTaskBtn.addEventListener("click", () => {
  ...
  days.forEach((d) => (taskObj.days[d] = false)); // unchecked by default
...
function generateSchedule() {
  ...
  // Clean up ghost people
  peopleList = peopleList.filter((p) => p.name && p.name.trim());
  saveData();
    ...
    days.forEach((day) => {
      if (task.days[day]) {
        const available = shuffle(
          peopleList.filter((p) => availability[p.name][day][task.name])
        );
        if (!available.length) return;
        if (!dayTaskCount[day]) dayTaskCount[day] = {};
        available.forEach((p) => {
          if (dayTaskCount[day][p.name] == null) dayTaskCount[day][p.name] = 0;
        });
        for (let i = 0; i < task.count; i++) {
          available.sort((a, b) => dayTaskCount[day][a.name] - dayTaskCount[day][b.name]);
          const p = available[0];
          const cellIdx = peopleList.findIndex((pl) => pl.name === p.name) + 2;
...
document.querySelector(".input-group").appendChild(resetBtn);
